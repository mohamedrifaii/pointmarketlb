...
export default function Home(){
 ...
 return <main>
  ...
  <section className="cartPanel" id="cart"><div><p className="eyebrow">YOUR BASKET</p><h2>{count?`${count} item${count>1?"s":""} ready`:"Your bag is waiting."}</h2><p>{count?"Review your selection before checkout.":"Add a few everyday essentials to get started."}</p></div><div className="total"><span>Estimated total</span><strong>${total.toFixed(2)}</strong><button disabled={!count} onClick={()=>alert("Demo checkout — payment will be connected next.")}>Checkout</button></div></section>
