...
function initChatPage() {
  ...
  const quickTopics = byId('chatQuickTopics');
  if (!thread || !composer || !input) return;

  const replies = [
    'Thanks for reaching out. Our team can help with that.',
    'For most Beirut orders, delivery is usually within 24-48 hours.',
    'Yes, we provide warranty support depending on the brand and product type.',
    'I can help you compare options if you share your budget and preferred category.',
  ];

  const addBubble = (text, sender = 'support') => {
    const bubble = document.createElement('div');
    bubble.className = sender === 'user'
      ? 'ml-auto max-w-[85%] rounded-2xl bg-primary text-on-primary px-4 py-3 text-sm font-medium'
      : 'mr-auto max-w-[85%] rounded-2xl bg-surface-container text-on-surface px-4 py-3 text-sm';
  ...
  const sendUserMessage = (text) => {
    ...
    window.setTimeout(() => {
      ...
      addBubble(answer, 'support');
    }, 450);
  };

  addBubble('Hi, welcome to Point Market LB support. What can we help you with today?', 'support');
  ...
  quickTopics?.addEventListener('click', (event) => {
    ...
    if (!button) return;
    sendUserMessage(button.dataset.chatTopic);
  });
}
...
function initContactPage() {
  ...
  form.addEventListener('submit', (event) => {
    ...
    showMessage(message, 'Thanks, your message was received. Our team will contact you soon.', 'success');
    form.reset();
  });
}
